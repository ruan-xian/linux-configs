...
,count5xx() {
    { echo "5xx Count    ,Route"; awk '/5[0-9]{2}\s+[0-9]+\s+[0-9]+ms/ {gsub(/[0-9a-f]{24}/, ".*", $(NF-3)); c[sprintf("%-8s%s", $(NF-4), $(NF-3))]++} END {for (k in c) print c[k] "," k}' /var/log/messages | sort -rn; } | column -t -s ","
    }
,get5xxContext() {
